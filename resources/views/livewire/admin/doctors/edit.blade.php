<div>
    @if ($show)
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4" wire:click.self="closeModal">
            <div class="bg-white rounded-xl p-6 max-w-2xl w-full max-h-[90vh] overflow-hidden flex flex-col">
                <div class="flex items-center justify-between mb-4">
                    <h3 class="font-fraunces text-lg text-forest">Edit Dokter</h3>
                    ...
                    <form wire:submit="save" class="space-y-5">
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <input type="text" wire:model="specialization" class="w-full rounded-lg border p-2 border-gray-300 focus:border-forest focus:ring-forest text-sm">
                        ...
                        <div>
                            ...
                            <textarea wire:model="bio" rows="3" class="w-full rounded-lg border p-2 border-gray-300 focus:border-forest focus:ring-forest text-sm"></textarea>
                        ...
                        <div>
                            ...
                            @if ($existingPhoto && ! $photo)
                                <img src="{{ \Storage::url($existingPhoto) }}" class="w-20 h-20 object-cover rounded-full mb-2">
                            @endif
                        ...
                        <div>
                            <div class="flex items-center justify-between mb-3">
                                ...
                                <button type="button" wire:click="addSchedule" class="text-xs text-white hover:underline bg-sky-400 p-2 rounded-md">
                                    + Tambah Jadwal
                                </button>
                            </div>
                            ...
                            <div class="space-y-3">
                                @forelse ($schedules as $index => $schedule)
                                    <div wire:key="edit-schedule-{{ $schedule['id'] ?? 'new-' . $index }}" class="flex flex-col sm:flex-row gap-2 items-start sm:items-center p-3 bg-ivory rounded-lg">
                                        <div class="w-full sm:w-32">
                                            <select wire:model="schedules.{{ $index }}.day" class="w-full rounded-lg border p-2 border-gray-300 text-xs focus:border-forest focus:ring-forest">
                                        ...
                                        <div class="w-full sm:w-28">
                                            <input type="time" wire:model="schedules.{{ $index }}.start_time" class="w-full rounded-lg border p-2 border-gray-300 text-xs focus:border-forest focus:ring-forest">
                                        ...
                                        <div class="w-full sm:w-28">
                                            <input type="time" wire:model="schedules.{{ $index }}.end_time" class="w-full rounded-lg border p-2 border-gray-300 text-xs focus:border-forest focus:ring-forest">
