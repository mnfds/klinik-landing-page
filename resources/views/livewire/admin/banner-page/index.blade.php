<div>
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-7">
        <div>
            ...
            <p class="text-sm text-charcoal/60 mt-0.5">Kelola banner untuk halaman dalam (services, products, dll).</p>
        </div>
        <button
            type="button"
    ...
    <div class="grid grid-cols-3 gap-3 mb-6">
        <div class="bg-white rounded-xl border border-blue-300 px-4 py-3 shadow-md">
            <p class="text-xs text-charcoal/50">Total Banner</p>
            <p class="text-xl font-fraunces text-forest mt-0.5">{{ $banners->total() }}</p>
        </div>
        <div class="bg-white rounded-xl border border-blue-300 px-4 py-3 shadow-md">
            <p class="text-xs text-charcoal/50">Aktif</p>
            <p class="text-xl font-fraunces text-forest mt-0.5">{{ $banners->where('is_active', true)->count() }}</p>
        </div>
        <div class="bg-white rounded-xl border border-blue-300 px-4 py-3 shadow-md">
            <p class="text-xs text-charcoal/50">Nonaktif</p>
            <p class="text-xl font-fraunces text-forest mt-0.5">{{ $banners->where('is_active', false)->count() }}</p>
        </div>
    </div>
    ...
    <div class="flex flex-col sm:flex-row gap-3 mb-4">
        <div class="relative w-full sm:w-64">
            <i class="fa-solid fa-sliders absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 text-sm pointer-events-none"></i>
            <select
                wire:model.live="filterType"
                class="w-full py-2 pl-9 rounded-xl border border-blue-300 text-sm focus:border-forest focus:ring-forest focus:ring-1 shadow-md appearance-none bg-white"
            >
    ...
    {{-- ================= DESKTOP: Tabel ================= --}}
    <div class="hidden md:block bg-white p-2 rounded-2xl shadow-sm overflow-hidden relative" wire:loading.class="opacity-60 pointer-events-none" wire:target="filterType">
            ...
            <table class="w-full text-sm">
                <thead class="bg-ivory/70 text-charcoal/60 text-left uppercase text-[11px] tracking-wider">
                    <tr>
                        ...
                        <th class="px-5 py-3.5 font-semibold">Halaman</th>
                ...
                <tbody class="divide-y divide-gray-50">
                    @forelse ($banners as $banner)
                        <tr wire:key="banner-page-{{ $banner->id }}" class="hover:bg-ivory/40 transition-colors group">
                            <td class="px-5 py-3.5">
                                <div class="flex items-center gap-3">
                                    @if ($banner->image_desktop)
                                        <img src="{{ \Storage::url($banner->image_desktop) }}" class="w-20 h-12 object-cover rounded-xl ring-1 ring-black/5">
                                    @else
                                        <div class="w-20 h-12 bg-gradient-to-br from-forest/10 to-gold/10 rounded-xl flex items-center justify-center text-forest/40 shrink-0">
                                    ...
                                    @endif
                                    <span class="font-medium text-charcoal max-w-xs truncate">{{ $banner->text_title ?? '—' }}</span>
                                </div>
                            </td>
                            <td class="px-5 py-3.5">
                                <span class="inline-flex items-center px-2.5 py-1 rounded-full text-[11px] font-medium bg-blush/40 text-forest">
                            ...
                            <td class="px-5 py-3.5">
                                <button
                                    ...
                                    class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium transition {{ $banner->is_active ? 'bg-green-50 text-green-700 hover:bg-green-100' : 'bg-gray-100 text-gray-500 hover:bg-gray-200' }}"
                                >
                                    <span class="w-1.5 h-1.5 rounded-full {{ $banner->is_active ? 'bg-green-500' : 'bg-gray-400' }}"></span>
                        ...
                        <tr>
                            <td colspan="4" class="px-4 py-16 text-center text-charcoal/50">
                                <div class="flex flex-col items-center gap-2.5">
                                    ...
                                    <p class="text-sm">Belum ada banner page.</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
    ...
    {{-- ================= MOBILE: Card list ================= --}}
    <div class="md:hidden space-y-3" wire:loading.class="opacity-60 pointer-events-none" wire:target="filterType">
        @forelse ($banners as $banner)
            <div wire:key="mobile-banner-page-{{ $banner->id }}" class="bg-white rounded-2xl p-4 shadow-sm">
                <div class="flex items-start gap-3">
                    @if ($banner->image_desktop)
                        <img src="{{ \Storage::url($banner->image_desktop) }}" class="w-20 h-14 object-cover rounded-xl ring-1 ring-black/5 shrink-0">
                    @else
                        <div class="w-20 h-14 bg-gradient-to-br from-forest/10 to-gold/10 rounded-xl flex items-center justify-center text-forest/40 shrink-0">
                    ...
                    <div class="flex-1 min-w-0">
                        <div class="flex items-start justify-between gap-2">
                            <p class="font-medium text-charcoal leading-snug truncate">{{ $banner->text_title ?? '—' }}</p>
                            <button
                                wire:click="toggleActive({{ $banner->id }})"
                                class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[11px] font-medium shrink-0 {{ $banner->is_active ? 'bg-green-50 text-green-700' : 'bg-gray-100 text-gray-500' }}"
                            >
                                <span class="w-1.5 h-1.5 rounded-full {{ $banner->is_active ? 'bg-green-500' : 'bg-gray-400' }}"></span>
                                {{ $banner->is_active ? 'Aktif' : 'Nonaktif' }}
                            </button>
                        </div>
                        <span class="inline-flex items-center px-2 py-0.5 rounded-full text-[11px] font-medium bg-blush/40 text-forest mt-1.5">
                            {{ $typeLabels[$banner->type] ?? $banner->type }}
                        </span>
                    </div>
                </div>
    ...
    @if ($deleteId)
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm px-4" wire:click.self="cancelDelete">
            <div class="bg-white rounded-2xl p-6 max-w-sm w-full shadow-2xl animate-[fadeIn_0.2s_ease]">
                ...
                <h3 class="font-fraunces text-lg text-forest mb-1.5">Hapus Banner?</h3>
