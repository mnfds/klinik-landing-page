<div>
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-7">
        <div>
            ...
            <p class="text-sm text-charcoal/60 mt-0.5">Kelola daftar promo & penawaran.</p>
        </div>
        <button
            type="button"
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-sm">
                <thead class="bg-ivory/70 text-charcoal/60 text-left uppercase text-[11px] tracking-wider">
                    <tr>
                        <th class="px-5 py-3.5 font-semibold">Promo</th>
                ...
                <tbody class="divide-y divide-gray-50">
                    @forelse ($promos as $promo)
                        <tr wire:key="promo-{{ $promo->id }}" class="hover:bg-ivory/40 transition-colors group">
                            <td class="px-5 py-3.5">
                                <div class="flex items-center gap-3">
                                    @if ($promo->image)
                                        <img src="{{ \Storage::url($promo->image) }}" class="w-11 h-11 object-cover rounded-xl ring-1 ring-black/5">
                                    @else
                                    ...
                                    @endif
                                    <span class="font-medium text-charcoal">{{ $promo->name }}</span>
                                </div>
                            </td>
                            <td class="px-5 py-3.5 text-xs text-charcoal/70">
                            ...
                            <td class="px-5 py-3.5">
                                <button
                                    ...
                                    class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium transition {{ $promo->is_active ? 'bg-green-50 text-green-700 hover:bg-green-100' : 'bg-gray-100 text-gray-500 hover:bg-gray-200' }}"
                                >
                                    <span class="w-1.5 h-1.5 rounded-full {{ $promo->is_active ? 'bg-green-500' : 'bg-gray-400' }}"></span>
    ...
    <div class="md:hidden space-y-3" wire:loading.class="opacity-60 pointer-events-none" wire:target="search">
        @forelse ($promos as $promo)
            <div wire:key="mobile-promo-{{ $promo->id }}" class="bg-white rounded-2xl p-4 shadow-sm">
                <div class="flex items-start gap-3">
                    @if ($promo->image)
                        <img src="{{ \Storage::url($promo->image) }}" class="w-14 h-14 object-cover rounded-xl ring-1 ring-black/5 shrink-0">
                    @else
                    ...
                    <div class="flex-1 min-w-0">
                        <div class="flex items-start justify-between gap-2">
                            <p class="font-medium text-charcoal leading-snug">{{ $promo->name }}</p>
                            <button
                                wire:click="toggleActive({{ $promo->id }})"
                                class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[11px] font-medium shrink-0 {{ $promo->is_active ? 'bg-green-50 text-green-700' : 'bg-gray-100 text-gray-500' }}"
                            >
                                <span class="w-1.5 h-1.5 rounded-full {{ $promo->is_active ? 'bg-green-500' : 'bg-gray-400' }}"></span>
                                {{ $promo->is_active ? 'Aktif' : 'Nonaktif' }}
                            </button>
                        </div>
                        <p class="text-xs text-charcoal/60 mt-1.5">
                            @if ($promo->start_date || $promo->end_date)
                                {{ $promo->start_date?->format('d M Y') ?? '-' }} &mdash; {{ $promo->end_date?->format('d M Y') ?? '-' }}
                            @else
                                —
                        ...
                        <p class="text-xs text-charcoal/60 tabular-nums mt-1">
                            {{ $promo->price ? 'Rp ' . number_format($promo->price, 0, ',', '.') : '—' }}
                        </p>
                    </div>
                </div>
    ...
    @if ($deleteId)
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm px-4" wire:click.self="cancelDelete">
            <div class="bg-white rounded-2xl p-6 max-w-sm w-full shadow-2xl animate-[fadeIn_0.2s_ease]">
                ...
                <h3 class="font-fraunces text-lg text-forest mb-1.5">Hapus Promo?</h3>
